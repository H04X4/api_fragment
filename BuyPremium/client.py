...
class FragmentClient:
    ...
    async def fetch_recipient(self, query):
        data = {"query": query,  "months": 3, "method": "searchPremiumGiftRecipient"}
    ...
    async def fetch_req_id(self, recipient, quantity):
        data = {"recipient": recipient, "months": 3, "method": "initGiftPremiumRequest"}
        async with httpx.AsyncClient() as client:
            response = await client.post(self.URL, cookies=get_cookies(DATA), data=data)
            print(response.json())
    ...
    async def fetch_buy_link(self, recipient, req_id, quantity):
        ...
        headers = {
            ...
            "referer": f"https://fragment.com/premium/gift?recipient={recipient}&months=3",
